...
<div>{Number.parseInt(slider) === 0 ? "No Limit" : `${slider} (minutes)`}</div>
      ...
      <TextContainer>
        {( heatingTimeout!=0 ) &&(<Text>heating Timeout hours:{new Date(heatingTimeout - new Date().getTime()).getHours()} minutes: {new Date(heatingTimeout - new Date().getTime()).getMinutes()} </Text>)}
