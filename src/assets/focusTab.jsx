...
<div>{Number.parseInt(slider) === 0 ? "No Limit" : `${slider} (minutes)`}</div>
      ...
      <TextContainer>
        {( heatingTimeout!=0 ) &&(<Text>heating Timeout H:{Math.floor((new Date(heatingTimeout - new Date().getTime()).getTime())/60)}s: {(new Date(heatingTimeout - new Date().getTime()).getTime()) % 60} </Text>)}
